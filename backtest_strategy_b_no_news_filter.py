"""
Backtest Strategy B WITHOUT news filter (baseline)
Uses same logic as adaptive_backtester.py
"""
...
def adaptive_backtest_no_news_filter(df, use_adaptive_targets=True):
    """
    Run backtest with adaptive targets/stops WITHOUT news filter
    ...
    """
    try:
        logger.info(f"Starting adaptive backtest on {len(df)} candles (NO news filter)...")
        ...
        for i in range(len(df)):
            ...
            # Check for new entry signals (NO news filter)
...
def main():
    """Main function"""
    logger.info("=" * 70)
    logger.info("BACKTESTING STRATEGY B WITHOUT NEWS FILTER (BASELINE)")
    logger.info("=" * 70)
    ...
    # Run backtest WITHOUT news filter
    trades = adaptive_backtest_no_news_filter(df, use_adaptive_targets=True)
    ...
    # Save results
    results_file = "results/backtest_results_strategy_b_no_news_filter.csv"
    os.makedirs(os.path.dirname(results_file), exist_ok=True)
    trades_df.to_csv(results_file, index=False)
    
    # Print results
    print("\n" + "=" * 70)
    print("STRATEGY B WITHOUT NEWS FILTER - BACKTEST RESULTS (BASELINE)")
    print("=" * 70)
