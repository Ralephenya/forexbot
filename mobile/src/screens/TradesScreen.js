      ...
      >
        <View style={styles.modal}>
          ...
          <ScrollView style={styles.modalBody}>
            <DetailRow label="Status" value={selected?.status} valueColor={selected?.status === 'OPEN' ? COLORS.profit : COLORS.textMuted} />
            <DetailRow label="Direction" value={selected?.direction} valueColor={selected?.direction === 'BUY' ? COLORS.buy : COLORS.sell} />
            ...
            <DetailRow label="P&L" value={formatPnl(selected?.pnl)} valueColor={pnlColor(selected?.pnl)} />
            <DetailRow label="Pips" value={selected?.pips !== null ? `${Number(selected?.pips || 0).toFixed(1)} pips` : '—'} valueColor={pnlColor(selected?.pips)} />
            <DetailRow label="Exit Reason" value={selected?.exit_reason} />
            <DetailRow label="Strategy" value={selected?.strategy_name || 'Strategy B'} valueColor={COLORS.accent} />
            <DetailRow label="Opened" value={selected?.timestamp ? new Date(selected.timestamp).toLocaleString() : '—'} />
